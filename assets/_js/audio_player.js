...
$(".player").each(function(){
  ...
  audio[0].addEventListener("play", function(){
    ...
    if (curr_title != '!@#$%') { // if a track is already playing
      new_title = document.title + " // " + audio.data("title");
    } else {
      new_title = "Now playing: " + audio.data("title");
    }
...
$("#playall").click(function(){
  ...
  $(".music.playing").each(function(){
    $(this).find("audio").each(function(){this.pause()});
  });
  // if tag(s) selected
  // otherwise reset player list, in case it had been updated previously
  if ($(".tag.active").length > 0){
